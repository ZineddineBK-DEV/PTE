...
import {
  ...
  acceptEvent,
  reset,
} from "../../features/userEvents/userEventSlice";

const TechTeamCalendar = () => {
  ...
  const { userEvents, isError, isSuccess, message } = useSelector(
  ...
  const eventHandleAdd = async (data) => {
    ...
    dispatch(setUserEvent(data)).then(() => {
  ...
  const handleDatesSet = (data) => {
    ...
    dispatch(getUserEvents(body));
  };

  function renderEventContent(eventInfo) {
    if (eventInfo.event._def.extendedProps.isAccepted) {
      return (
        <div className="relative inline-flex w-full h-full justify-start px-2 py-1 overflow-hidden font-medium transition-all bg-green-500 rounded-xl group cursor-pointer">
      ...
      return (
        <div className="relative inline-flex w-full h-full justify-start px-2 py-1 overflow-hidden font-medium transition-all bg-red-500 rounded-xl group cursor-pointer">
          <span className="absolute top-0 right-0 inline-block w-4 h-4 transition-all duration-500 ease-in-out bg-red-700 rounded group-hover:-mr-4 group-hover:-mt-4">
            <span className="absolute top-0 right-0 w-5 h-5 rotate-45 translate-x-1/2 -translate-y-1/2 bg-white"></span>
          </span>
          <span className="absolute bottom-0 left-0 w-full h-full transition-all duration-500 ease-in-out delay-200 -translate-x-full translate-y-full bg-red-600 rounded-2xl group-hover:mb-12 group-hover:translate-x-0"></span>
          <span className="relative w-full inline-flex text-left text-white transition-colors duration-200 ease-in-out group-hover:text-white">
            <MdPendingActions className="text-white pt-1 text-2xl" />{" "}
            <i className="capitalize font-base text-white pt-1">
              {eventInfo.event.title}
            </i>
          </span>
  ...
  const onEventClick = (eventInfo) => {
    var eventData = userEvents.filter((ev) => {
    ...
    return (
      <div className="bg-white p-10 rounded-lg font-medium">
        ...
        <div className="inline-flex pl-2 pt-3 capitalize">
          ...
          <h1 className="  pl-3 underline text-gray-600">
        ...
        <div className="inline-flex pl-2  capitalize">
          <div className="pt-1">
            <GrUserWorker className="shadow-xl text-gray-500" />
          </div>
          <h1 className="  pl-3 underline text-gray-600">Job </h1>
          <h1 className=" capitalize pl-3 ">: {props.event.job} </h1>
        </div>
        <br />
        {/* destination */}
        <div className="inline-flex pl-2 capitalize">
          ...
          <h1 className="pl-2 text-gray-600 underline">Address </h1>
          <h1 className="pl-2">: {props.event.address} </h1>
        </div>
        <br />
        {/* Start */}
        <div className="inline-flex pl-2 capitalize">
        ...
        <div className="grid grid-cols-2 gap-1 mt-5">
          {(user.roles.includes("admin") ||
            props.event.applicant._id === user.id ||
            props.event.engineer === user.id) && (
            <button
              onClick={() => onDeleteEvent(props.event._id)}
              className="rounded px-5 py-2.5 overflow-hidden group bg-rose-500 relative hover:bg-gradient-to-r hover:from-rose-500 hover:to-rose-400 text-white hover:ring-2 hover:ring-offset-2 hover:ring-rose-400 transition-all ease-out duration-300  float-left"
            >
              <span className="absolute right-0 w-8 h-32 -mt-12 transition-all duration-1000 transform translate-x-12 bg-white opacity-10 rotate-12 group-hover:-translate-x-40 ease"></span>
...
export default TechTeamCalendar;
